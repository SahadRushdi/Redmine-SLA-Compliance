...
module Sla
  # Step 6.3 — per-priority effective met/breached counts for the tickets-by-priority
  ...
  class PriorityBreakdown
    Row = Struct.new(:priority_id, :priority_name, :met, :breached, :at_risk, :total,
                     keyword_init: true)
    ...
    def call
      ...
      IssuePriority.where(id: by_priority.keys).sorted.map do |priority|
        ...
        Row.new(priority_id: priority.id, priority_name: priority.name,
                ...
                at_risk: row.at_risk_count.to_i,
                total: row.met_count.to_i + row.breached_count.to_i)
      end
    end
  end
end
